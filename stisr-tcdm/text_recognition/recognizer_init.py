...
def CRNN_init(model_path):
    ...
    stat_dict = torch.load(model_path, weights_only=False) #Modified
...
def Aster_init(model_path):
    ...
    # Load checkpoint with nested state_dict support
    checkpoint = torch.load(model_path)
    if 'state_dict' in checkpoint:
        sd = checkpoint['state_dict']
        if 'state_dict' in sd:
            sd = sd['state_dict']
        ASTER.load_state_dict(sd)
    else:
        ASTER.load_state_dict(checkpoint)
