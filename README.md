# lpr-tcdm
An application of the "Scene Text Image Super-resolution based on Text-conditional Diffusion Models" paper on the RLPR dataset.
...
As the images needed for running the super-resolver have already been generated as .npz files, it is not necessary to download the above datasets for the purpose of this project unless you plan on training the models from scratch.
...
------------------------------------
1. Training Synthesizer and Degrader
...
2. Running models: To run the Synthesizer, run the script via: bash run_synthesizer.sh. The generated text images and the corresponding text labels are placed in "./diff_samples/mr_samples".
...
Generating LR and HR text images: To run Super-resolver on the TextZoom dataset, run the script via: bash run_super_resolver.sh. This originally used the MR images in "/diff_samples/mr_samples", but the model in the project has used the resized MR samples in "/diff_samples/mr_samples/resized-64x128".
To run Super-resolver on the RLPR dataset, run the script via: bash run_sr_rlpr.sh. 

Evaluation
----------
To evaluate the results for the super-resolved TextZoom images, you will need the .npz files generated by the GT-DiMSS model via running the script: run bash eval_gt_dimss_textzoom.sh. For this project, the MR image samples have already been placed in "/diff_samples/textzoom" and titled with their corresponding difficulty level, such as easy_sr_samples_gt_dimss.npz. Then, run evaluate.py
...
Videos of running the super-resolver and evaluating the results on the TextZoom and RLPR datasets have been added to the .zip file.
